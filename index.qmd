...
## Algoritmos de compressão — como funcionam? {.smaller}

> **Conceito fundamental**: A compressão é identificar e eliminar redundância — informação que pode ser reconstruída a partir de outra informação.

---

### Exemplo 1: Padrões matemáticos
...
**Comprimido** (padrão identificado):
```
...
### Exemplo 2: Texto clínico — Run-Length Encoding (RLE)
...
### Técnicas reais nos formatos

| Técnica | Princípio | Formato |
|---------|-----------|---------|
| **RLE** | Codifica repetições (`AAAA` → `A×4`) | BMP, TIFF |
| **Huffman** | Códigos curtos para símbolos frequentes | ZIP, JPEG, MP3 |
| **DCT** | Descarta frequências imperceptíveis | JPEG (**lossy**) |
| **LZMA** | Dicionário de padrões recorrentes | 7z, ZIP |

::: {.notes}
**RLE (Run-Length Encoding)**: Algoritmo mais simples. Eficaz em imagens com áreas de cor uniforme (ex: diagramas, ícones). Ineficaz em fotografias complexas.

**Huffman coding**: Baseado na frequência estatística. Se "e" aparece 15% das vezes em português, recebe código binário curto (ex: "01"). Se "z" aparece 0,1%, recebe código longo (ex: "111010"). O resultado é um ficheiro mais compacto sem perda de informação — por isso é usado em compressão lossless.

**DCT (Discrete Cosine Transform)**: Usado no JPEG. Transforma a imagem em componentes de frequência e descarta as altas frequências (detalhes finos) que o olho humano percebe menos. Como a informação descartada não pode ser recuperada, o JPEG é **lossy**. Veremos nas próximas slides o impacto clínico concreto desta escolha.
:::
...
- Dúvidas sobre o exercício: [tiagojacinto@med.up.pt](mailto:tiagojacinto@med.up.pt)
